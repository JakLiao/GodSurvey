...
public class PaperDaoImpl extends BaseDao implements PaperDao {
	...
	public List<Paper> findAllPapers() {
		...
		try {
			while (rs.next()) {
				...
				paper.setPaperEndDate(new SimpleDateFormat(
						"yyyy-MM-dd HH:mm:ss").format(rs.getDate(6)));
	...
	public List<Paper> findPaperByUserId(int userId) {
		...
		try {
			while (rs.next()) {
				...
				paper.setPaperTitle(rs.getString(3));
				paper.setPaperSummary(rs.getString(4));
				paper.setPaperStartDate(new SimpleDateFormat(
						"yyyy-MM-dd HH:mm:ss").format(rs.getDate(5)));
				paper.setPaperEndDate(new SimpleDateFormat(
						"yyyy-MM-dd HH:mm:ss").format(rs.getDate(6)));
				paper.setPaperBg(rs.getString(7));
				paper.setPaperType(rs.getInt(8));
	...
	public int addPapers(Paper paper) {
		...
		doOperate(sql, paper.getUserId(), paper.getPaperTitle(),
				paper.getPaperSummary(), paper.getPaperStartDate(),
				paper.getPaperEndDate(), paper.getPaperBg(),
				paper.getPaperType(), paper.getPaperStatus(),
				paper.getPaperCount());
		closeAll();
